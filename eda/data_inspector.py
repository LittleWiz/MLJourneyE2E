...
logger = setup_logger("E2EML", log_level=logging.INFO, console_level=logging.ERROR, stage_name="INSPECTION")
...
class DataInspector:
    ...
    def null_percentages(self):
        ...
        logger.info(f"Null percentages per column:\n{null_percent}")
        return null_percent
    ...
    def nulls_by_source_file(self):
        ...
        if 'source_file' not in self.df.columns:
            logger.warning("Column 'source_file' not found for per-file null analysis.")
            return None
...
# inspector = DataInspection(df)
# inspector.data_shape()
# inspector.data_types()